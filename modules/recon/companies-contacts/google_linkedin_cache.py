...
class Module(BaseModule, GoogleAPIMixin):
    ...
    meta = {
        'name': 'Google Cache Linkedin Profile and Contact Harvester',
        'author': 'Jan-Jaap Korpershoek (@JJK96), Joe Black (@MyChickenNinja), @fullmetalcache, and Brian King',
        'version': '1.0',
        'description': 'Harvests profiles from LinkedIn by querying the Google API cache for LinkedIn pages related to the given companies, and adds them to the \'profiles\' table. The module will then parse the resulting information to extract the user\'s full name and job title (title parsing recently improved). The user\'s full name and title are then added to the \'contacts\' table. This module does not access LinkedIn at any time.',
        ...
        'options': (
            ...
            ('extra_terms', None, False, 'Extra search terms to include'),
        ),
    }
    ...
    def get_profiles(self, company):
        ...
        extra_terms = self.options['extra_terms']
        for subdomain in subdomain_list:
            query = f'site:{subdomain}linkedin.com/in "{company}"'
            if extra_terms:
                query += ' ' + extra_terms
            results = self.search_google_api(query, self.options['limit'])
    ...
    def parse_jobtitle(self, company, snippet):
        ...
        if ' at ' in snippet and 'See who you know' not in snippet and company in snippet.lower() and len(chunks) < 3:
